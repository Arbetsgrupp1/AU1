...
% ===============================================================    
    case 'start'
        ...
        while (get(axHndl,'Userdata')>=play) && (X(1,1) < 100) ...
           ...
           if (docked == -1)
                text(-20,0,5,' Elasitc Collision');   % Signal succesful docking
            end
...
% V(1,1)=(F(1,1).*dt)./M(1,1); % den nya hastighet
% XNEW = X + V.*dt; % den nya läget för satellite 1
% VNEW = (X./dt);
% % acc1 = F(1,1)/M(1,1);
% % acc2 = F(1,2)/M(2,2); 
% 
...
XNEW = X; 
    ...
    acc1 = F(1,1)/M(1,1);
    VNEW(1,1) = V(1,1) + acc1*dt;  
    XNEW(1,1) = X(1,1) + VNEW(1,1)*dt;

if(docked == 0)
    if(abs( XNEW(1,1) - XNEW(1,2) ) <= 5) %Om avståndet mellan de två satelliterna är 0-5
       if( V(1,1)-V(1,2) <= 2) %Om hastigheten för sat.1 är <= 2m/s
         docked = 1; 
       else
         docked = -1;
       end
  ...
  if (docked == 1) %Oelastisk kollision
          VFinal = (M(1,1)/(M(1,1)+M(2,2)))*VNEW(1,1); % Sluthastighet 
          V(1,1) = VFinal; %Ny hastighet för sat.1
          V(1,2) = VFinal; %Ny hastighet för sat.2
%           accDock = F(1,1)/( M(1,1)+M(2,2) );
%           VNEW(1,1) = V(1,1) + accDock*dt;  
%           VNEW(1,2) = VNEW(1,1);  
          
          XNEW(1,1) = X(1,1) + VFinal*dt; %Ny position för sat.1
          XNEW(1,2) = X(1,2) + VFinal*dt; %Ny position för sat.2   
  end
  
  if (docked == -1) %Elastisk kollision     
        VFinalSat1 = ( ( M(1,1)-M(2,2) ) / ( M(1,1)+M(2,2) ) ) * VNEW(1,1); %Sluthastighet för sat.1 
        VFinalSat2 = ( ( 2*M(1,1) ) / ( M(1,1)+M(2,2) ) ) * VNEW(1,1);
        V(1,1) = VFinalSat1; %Ny hastighet för sat.1
        V(1,2) = VFinalSat2; %Ny hastighet för sat.2
        
        XNEW(1,1) = X(1,1) + V(1,1)*dt; %Ny position för sat.1
        XNEW(1,2) = X(1,2) + V(1,2)*dt; %Ny position för sat.2
        docked = 2;
